...
```bash
conda create -n ml_prod_deployment python=3.9
conda activate ml_prod_deployment
```
...
```
This command will create your personal repository under `/Repos/<username>/ml_prod_deployment`.
...
- To trigger the release pipeline, get the current version from the `ml_prod_deployment/__init__.py` file and tag the current code version:
```
